...
class UiBoard(Entity):
    def __init__(self, name, x, y):
        ...
        self.line_1 = Entity("line-1", (x * scale + padding + 15, y * scale + 15, z_order))
        self.line_1.add(Engine.get().text("", 20, self.line_1))

        self.line_2 = Entity("line-2", (x * scale + padding + 15, y * scale + 55, z_order))
    ...
    def add_message(self, message):
        ...
        if len(self.messages) > 0:
            Engine.get().emit(Event("settext", target=self.line_2, text=self.messages[-1]))
        if len(self.messages) > 1:
            Engine.get().emit(Event("settext", target=self.line_1, text=self.messages[-2]))
